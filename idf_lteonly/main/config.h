...
#define PIN_LTE_RX     4     // 구모듈 swap (신모듈 native=2). [sss+구모듈, 2026-07-13]
#define PIN_LTE_TX     2     // 구모듈 swap (신모듈 native=4). [sss+구모듈, 2026-07-13]
#define PIN_BUZZER     1     // 액티브(마그네틱) 부저 — digitalWrite HIGH=on

// ── LTE 극성 (aa fork — 03_4 진단 확정값) ────────────────────────
#define LTE_DTR_IDLE      LOW    // [aa+신모듈 2026-07-13] DTR LOW 유지 (신모듈 native = DTR_ACTIVE LOW).
...
// ★[콜드 스트레서 분산 2026-07-13] 모뎀 전원(railOn)을 이만큼 늦춤 = ESP 부팅+RTC캘리 취약창과 시간분리.
//   콜드-스타트 크래시 루프 = 첫 ~수초에 (ESP RTC캘리 busy-wait) × (모뎀 인러시+어태치) × (충전IC 기동)
//   이 겹치는 것. RTC캘리 끝나고 레일 안정 후 모뎀 인러시가 오게 지연. A/B: 0=지연없음(baseline).
#define MODEM_PWR_DELAY_MS    10000  // 모뎀 railOn 전 대기(ms). 0=즉시(baseline), 10000=10초 지연빌드.
...
// ── sleep_mgr (Block 8) ──────────────────────────────────────────
#define SLEEP_DISABLED             1                  // [진단본] 절전 OFF (계속 깨어 관측). 운영본=0(절전ON).
...
#define HWDIAG_ENABLED             1                  // [진단본, 전달용] 크래시카운터/배너/부저 ON. 운영본=0.
