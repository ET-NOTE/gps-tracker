...
curl -sS -H "Authorization: Bearer $TOKEN" \
  "https://seriallog.com/gps-tracker/api/v1/devices/$DEVICE_ID/locations?limit=5" \
  | python3 -m json.tool

echo
echo "=== same with fix_only=true ==="
curl -sS -H "Authorization: Bearer $TOKEN" \
  "https://seriallog.com/gps-tracker/api/v1/devices/$DEVICE_ID/locations?limit=5&fix_only=true" \
  | python3 -m json.tool
