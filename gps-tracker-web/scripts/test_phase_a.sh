...
echo
echo "=== GET /auth/me ==="
curl -sS -H "Authorization: Bearer $TOKEN" https://gps.serial.kr/api/v1/auth/me | python3 -m json.tool
...
curl -sS -X PATCH -H "Authorization: Bearer $TOKEN" \
  ...
  https://gps.serial.kr/api/v1/auth/me | python3 -m json.tool
...
curl -sS -o /dev/null -w "wrong_current_pw → %{http_code}\n" \
  ...
  https://gps.serial.kr/api/v1/auth/me/password
...
curl -sS -H "Authorization: Bearer $TOKEN" \
  https://gps.serial.kr/api/v1/notifications/settings | python3 -m json.tool
...
curl -sS -H "Authorization: Bearer $TOKEN" https://gps.serial.kr/api/v1/devices | python3 -m json.tool
