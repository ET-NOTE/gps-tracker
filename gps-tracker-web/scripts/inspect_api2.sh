...
curl -sS -H "Authorization: Bearer $TOKEN" \
  "https://gps.serial.kr/api/v1/devices/$DEVICE_ID/locations?limit=3" | python3 -m json.tool

echo
echo "=== GET /devices/$DEVICE_ID/locations?limit=3&fix_only=true ==="
curl -sS -H "Authorization: Bearer $TOKEN" \
  "https://gps.serial.kr/api/v1/devices/$DEVICE_ID/locations?limit=3&fix_only=true" | python3 -m json.tool
