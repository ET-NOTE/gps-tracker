...
//   brPhase_ 0=idle, 1=등록대기. 매 호출 CEREG 1회만 폴 → loop 를 90초 굶기지 않음.
//   recovery 가 bringInProgress() 시 REG_POLL_FAST_MS 로 빠르게 재호출, 그 사이 GPS/모션 서비스.
static uint8_t  brPhase_ = 0;
static uint32_t brRegT0_ = 0;
bool bringInProgress() { return brPhase_ != 0; }
void bringReset()      { brPhase_ = 0; }   // 전원사이클/soft 후 Phase1 부터 재시작

bool bringUp() {
  bc::set("lte_bringup");

  if (brPhase_ == 0) {
    // ── Phase 1: AT/CPIN/CSQ (짧은 블로킹 ~수초). AT 무응답이면 즉시 실패(in-progress 아님) ──
    ...
    brPhase_ = 1; brRegT0_ = millis();
    return false;   // 등록 대기 진입 (in-progress)
  }

  // ── Phase 2: 등록 대기 — 매 호출 CEREG 1회만 폴 (non-blocking) ──
  if (millis() - brRegT0_ > LTE_REG_WAIT_MS) { brPhase_ = 0; return false; }   // 등록 timeout → 실패
  if (sendAT("AT+CEREG?", "OK", 2000)) {
    int p = lastResp.indexOf("+CEREG:");
    if (p >= 0) {
      int comma = lastResp.indexOf(',', p);
      if (comma >= 0) reg_ = lastResp.substring(comma + 1).toInt();
    }
  }
  if (reg_ != 1 && reg_ != 5) {
    Serial.printf("[LTE] waiting reg… csq=%d reg=%d (%lus)\n",
      csq_, reg_, (unsigned long)((millis() - brRegT0_) / 1000));
    return false;   // 계속 대기 (in-progress 유지)
  }
...
void refresh() {
  if (sendAT("AT+CSQ", "OK", 1500)) {     // [fix#4] expect=OK (토큰매칭 race → csq_=0 회피)
    int p = lastResp.indexOf("+CSQ:");
    if (p >= 0) csq_ = lastResp.substring(p + 5).toInt();
  }
  if (sendAT("AT+CEREG?", "OK", 1500)) {  // [fix#4] expect=OK (토큰매칭 race → reg_ 오설정 회피)
    int p = lastResp.indexOf("+CEREG:");
...
bool softReset() {
  ...
  brPhase_ = 0;   // (2026-07-08) bringUp phase 리셋 → 다음엔 Phase1(AT)부터
