...
static uint32_t activityEma_  = 0;   // (2026-07-08) raw 자력 |Δ| EMA (mg) — 활동량
static uint32_t activeLastMs_ = 0;   // 마지막으로 활동량이 임계 초과한 시각 (still 판정)
...
void tick() {
  ...
  // (2026-07-08) 활동량 EMA — raw 자력 |Δ|(중력은 Δ 로 상쇄). 지속 진동=주행/터널, 단발=정지 노이즈.
  {
    static uint32_t actSampleMs = 0;
...
bool readRaw(int16_t &xMg, int16_t &yMg, int16_t &zMg) {
  ...
  // [fix#11] I2C wedge 면 전 바이트 0xFF → 데이터로 오인 시 magnitude ~1mg(실제 ~1000mg) → EMA |Δ| 스파이크
  //   로 false-active. 전부 0xFF 면 실패 반환 → 호출부(mag>=0 체크)가 그 샘플 스킵.
  if (xl==0xFF && xh==0xFF && yl==0xFF && yh==0xFF && zl==0xFF && zh==0xFF) return false;
...
uint32_t activityMg() { return activityEma_; }
bool     active()     { return activityEma_ >= MOTION_ACTIVITY_THS_MG; }
uint32_t stillMs()    { return millis() - activeLastMs_; }   // activeLastMs_==0 → uptime(=매우 still)
