...
#define PIN_LTE_RX     4     // [구모듈 swap] LTE단독 진단용
#define PIN_LTE_TX     2     // [구모듈 swap]
#define PIN_BUZZER     1     // 액티브(마그네틱) 부저 — digitalWrite HIGH=on

// ── LTE 극성 (aa fork — 03_4 진단 확정값) ────────────────────────
#define LTE_DTR_IDLE      HIGH   // [구모듈 swap] DTR 반전
...
#define OBSERVE_MODE               1                  // sss 필드-유사 드라이브 관찰: sleep 행동은 필드처럼(안 자게 3분창) + wake원인 로깅 + 로그 유지(블로킹CDC). 운영본(aa)은 0.
