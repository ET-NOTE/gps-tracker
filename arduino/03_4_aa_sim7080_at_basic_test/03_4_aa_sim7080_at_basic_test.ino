// 03_4_aa — SIM7080G LTE 진단 (aa 전용 fork).
//
// 원본 03_4 (sss) 와 차이:
//   1. PWRKEY 극성 반전 (idle=LOW, pulse=HIGH → LOW). sss 는 idle=HIGH, pulse=LOW.
//   2. loop raw NMEA line 출력 활성화 (Serial.printf "[LTE] %s") — 응답 raw 확인.
//   3. loop 순환 AT 명령 (CPIN/CSQ/CEREG/CGATT/CGDCONT/CGPADDR) — 네트워크 registration/IP 확인.
//   DTR 은 LOW (sss 와 동일) — HIGH 세팅 시 SIM7080 이 sleep 진입 신호로 해석.
// sss 로 sketch 재사용은 원본 03_4 (PWRKEY 원래 극성 + raw 주석 + AT 단일 명령) 그대로 사용.
//
...
#define PIN_PWRKEY  7
#define PIN_DTR    10
#define PIN_LTE_RX  2   // ESP 받는 쪽 (SIM TX -> ESP RX)
#define PIN_LTE_TX  4   // ESP 보내는 쪽 (ESP TX -> SIM RX)
...
static void pulsePwrKey() {
  // (2026-07-01 aa 수리 후) PWRKEY 논리 반전 — idle LOW, pulse HIGH 1.5s → LOW.
...
static void powerOnSequence() {
  ...
  // (2026-07-01 aa 진단) DTR 재-원복: 첫 AT 응답 후 침묵 → DTR HIGH 가 sleep 진입 신호로 추정.
  // SIM7080 datasheet: DTR LOW=active(정상), HIGH=sleep 진입. LOW 로 원복.
  pinMode(PIN_DTR, OUTPUT);
  digitalWrite(PIN_DTR, LOW);
  Serial.println(F("[PWR] (b) DTR -> LOW (원복)"));

  // (c) 모듈 internal power 안정화 시간 (13_1 의 LIS3DH init 등 다른 작업이 차지하는 ~500ms 대용)
  delay(500);

  // (d) PWRKEY idle LOW (2026-07-01 aa 반전)
  pinMode(PIN_PWRKEY, OUTPUT);
  digitalWrite(PIN_PWRKEY, LOW);
  delay(200);
...
void loop() {
  ...
  while (lte.available()) {
    ...
    if (c == '\n' || c == '\r') {
      if (lineLen > 0) {
        line[lineLen] = 0;
        // (2026-07-01 aa 진단) raw 라인 출력 활성화 — 네트워크 등록/신호/IP 응답 눈으로 확인.
        Serial.printf("[LTE] %s\n", line);
  ...
  // (2026-07-01 aa 진단) 5초마다 순환 AT 명령 — 부팅/SIM/신호/등록/네트워크 attach/IP 순서 확인.
  static uint32_t lastAT = 0;
  static uint8_t  cmdIdx = 0;
  static const char* CMDS[] = {
    "AT",             // baseline
    "AT+CPIN?",       // SIM lock 상태 (+CPIN: READY 기대)
    "AT+CSQ",         // 신호 세기 (+CSQ: rssi,ber ; rssi=99 = 신호 없음)
    "AT+CEREG?",      // EPS 등록 (+CEREG: 0,1 or 0,5 = registered)
    "AT+CGATT?",      // packet attach (+CGATT: 1 = attached)
    "AT+CGDCONT?",    // PDP context (APN 확인)
    "AT+CGPADDR",     // IP 할당 여부
  };
  const uint8_t N_CMDS = sizeof(CMDS) / sizeof(CMDS[0]);
  if (now - lastAT >= 5000) {
    lastAT = now;
    Serial.printf("[TX] %s\n", CMDS[cmdIdx]);
    lte.print(CMDS[cmdIdx]);
    lte.print("\r\n");
    cmdIdx = (cmdIdx + 1) % N_CMDS;
  }
