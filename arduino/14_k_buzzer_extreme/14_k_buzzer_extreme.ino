...
#define PIN_PWR_EN     6
#define BUILD_TAG      "14_k"   // [diagnostic] /diagnostic sketch ID
#define PIN_PWRKEY     7
#define PIN_DTR        10
#define PIN_BAT        3
...
void setup() {
  ...
  // [14_k extreme] LTE bringup 내내 GPIO1 2700Hz PWM 연속 — PWM 노출 100배 (vs 14_j 400ms boot beep).
  // duration=0 = 무한, noTone() 호출까지 안 멈춤.
  tone(PIN_BUZZER, BUZZER_FREQ);
  ltePowerOn();
  bool lteOk = lteBringUp();
  noTone(PIN_BUZZER);
  digitalWrite(PIN_BUZZER, LOW);
  if (lteOk) {
    fetchSimInfo();
