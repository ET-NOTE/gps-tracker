...
// 1Hz × 30s = ~30 entries. (2026-06-29) BUF 60 → 120 — retry 시 4 POST 연속 실패까지 0 손실 보존 (이전 2 POST 한계). RAM +780B.
...
// (2026-07-01) Timer wake heartbeat 모드 — 첫 성공 POST 후 즉시 re-sleep. 5분 유지 안 함.
static bool timerWakeMode = false;
// (2026-07-01) Timer wake 최대 지속 — LTE 실패해도 2분 지나면 즉시 sleep. 지하 등 두절 지역
// 매 10분 사이클 마다 5분+ 소모 방지.
...
static void doPost() {
  ...
  if (ok) {
    ...
    if (status == 200) {
      ...
      // (2026-07-01) Timer wake heartbeat 완료 — 즉시 re-sleep.
      if (timerWakeMode) {
...
static void enterDeepSleep(const char *reason) {
  ...
  // (2026-07-01) timer wake heartbeat sleep 은 조용히 — 매 10분 실외 삐삐 방지.
  ...
  if (!lisOk) {
    DBGLN(F("[SLEEP] LIS not OK — abort sleep (no wake source)"));
    inSleepProcedure = false;
    return;
  }
...
void setup() {
  ...
  if (wc == ESP_SLEEP_WAKEUP_GPIO) {
    ...
    // (2026-07-01) 10분 timer wake — LIS 안 잡히는 케이스 heartbeat. 첫 성공 POST 후 즉시 re-sleep.
    wakeReasonStr = "timer";
    timerWakeMode = true;
  } else {
    // (2026-07-01) wc == UNDEFINED (재부팅) 은 rr 로 세분화 — 이전엔 전부 "boot" 로 통합되어
    // cold boot vs esp_restart (PR #108) vs brownout vs crash 구분 안 됨.
...
void loop() {
  ...
  if (timerWakeMode && (millis() - bootMs) > TIMER_WAKE_MAX_MS) {
    DBGLN(F("[TIMER-WAKE] 2분 timeout — 성공 POST 없이 re-sleep"));
    timerWakeMode = false;
