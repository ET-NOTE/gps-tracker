// =================================================================
// 15_field_v2 — 동료 필드 휴대용(aa) 스냅샷 (2026-07-03).
//   15_a_modular 최신(P0/P1/P1.5/P3/P4 + 모션 HPM=00 + 정지판정 + breadcrumb
//   + 부저맵[boot1/sleep2/wake6/fix3/post4/srv5])을 FIELD 설정으로 스냅샷:
//     OBSERVE_MODE 0(정지5분→sleep), TIMER_WAKE_ENABLED 0(모션 wake only),
//     Serial.setTxTimeoutMs(0)(non-blocking CDC). FQBN CDCOnBoot=cdc. 배터리 구동.
//   ※ 안테나(외부)는 HW LED 전류소모 원인 해결됨(GPS_ANTENNA.md). aa L86@9600.
// -----------------------------------------------------------------
...
void setup() {
  ...
  bc::set("setup");
  Serial.begin(115200);
  Serial.setTxTimeoutMs(0);   // ★ FIELD: 리더 없으면 로그 drop(non-blocking) → CDC-full 로 인한 LTE stuck 방지
  delay(200);
  Serial.println();
  Serial.println(F("=== 15_field_v2 (aa 필드본) — motion-wake only, non-blocking CDC ==="));
