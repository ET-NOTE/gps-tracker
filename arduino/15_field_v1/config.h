...
#define BUZZER_ENABLED 1   // (2026-07-02) 사용자 지시로 활성화. ⚠️ 마그네틱 부저(GPIO1)가 LTE bringup 방해 가능 — LTE stuck 관찰 시 confound 주의. 운영 전 정책 결정.
...
#define PWR_CYCLE_OFF_MS      2000   // railCycle: OFF 유지 (capacitor discharge)
...
// ── sleep_mgr (Block 8) ──────────────────────────────────────────
#define SLEEP_DISABLED             0                  // 1=자동 sleep 비활성 (연속 테스트용)
#define SLEEP_TEST                 0                  // 1=벤치 sleep/wake 검증용 단축값 (운영 전 0 원복!)
#define OBSERVE_MODE               0                  // FIELD 빌드: 운영 sleep 스케줄 (정지 5분→sleep, 10분 timer wake)
...
//   ⚠️ trade-off: LIS 가 차량 진동 놓치면 영구 sleep 가능 (사용자 의도적 선택). 관찰 필요.
#define TIMER_WAKE_ENABLED         0
#define WAKE_BEEP_COUNT            6                  // (FIELD) 모션 wake 시 부저 6회
...
#define LIS_MOT_THS            0x08    // 128mg threshold
