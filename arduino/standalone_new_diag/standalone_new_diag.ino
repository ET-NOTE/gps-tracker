...
// (2026-07-08 진단) reset-reason NVS 누적 — 배터리 재부팅(USB 없이)도 기록, STATUS 에 상시 표시.
...
void setup() {
...
#if HWDIAG_ENABLED
  {
    ...
    Serial.println("========================================================");
    Serial.flush();
    if (isCrash) { buzzer::beep(3, 500, 250); buzzer::flush(); }         // ★ 크래시 알람 = 긴 삐 3회 (모션wake 빠른6회와 구분, 2026-07-08)
