...
void tick() {
  ...
  if (!lte::ready()) {
    ...
    if (lastSuccessMs_ == 0 && (now - bootMs_) > BOOT_STUCK_RESTART_MS) {
      if (lte::csq() != 99) restart("boot-stuck (signal present, no POST)");
    }
    ...
    if (bringFails_ >= BRINGUP_FAIL_HARD) {
      ...
      if (now - lastHardCycleMs_ >= HARDCYCLE_MIN_INTERVAL_MS) {
        ...
        Serial.printf("[REC] hardCycle back-off (%lus 남음, 인러시 억제) — soft 재시도만\n",
          (unsigned long)((HARDCYCLE_MIN_INTERVAL_MS - (now - lastHardCycleMs_)) / 1000));
      }
    }

    buzzer::beep(1, 50, 0); buzzer::flush();   // (2026-07-08) LTE 재시도마다 짧은 부저 1회(50ms) — 운행 중 재시도 청각 인지 (다른 패턴과 구분: 단발 50ms)
