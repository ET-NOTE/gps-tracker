...
#include <WiFi.h>
#include <esp_sleep.h>
#include <driver/gpio.h>   // [14_l] gpio_hold_en for buzzer pin during deep sleep
...
#define PIN_PWR_EN     6
#define BUILD_TAG      "14_m"   // [diagnostic] /diagnostic sketch ID
#define PIN_PWRKEY     7
#define PIN_DTR        10
#define PIN_BAT        3
...
static void enterDeepSleep(const char *reason) {
  ...
  // [14_l SAFE SLEEP SEQUENCE] AI 분석 권장 — back-EMF spike 방지.
  noTone(PIN_BUZZER);                                // PWM hard stop
  pinMode(PIN_BUZZER, OUTPUT);
  digitalWrite(PIN_BUZZER, LOW);                     // 코일 한쪽 short → discharge path
  delay(20);                                         // discharge time
  gpio_hold_en((gpio_num_t)PIN_BUZZER);              // pin LOW 고정 (RTC domain hold)
  gpio_deep_sleep_hold_en();                         // hold 가 deep sleep 동안 유지
...
// =================================================================
void setup() {
  // [14_l] deep sleep wake 후 hold 해제 — pinMode/tone() 가 다시 동작하도록.
  ...
  // [14_m] cold boot / wake beep 모두 제거 — LTE bringup 직전 PWM spike 차단.
  // pin 만 LOW 로 hard 고정. 다른 milestone beep (first fix/post, sleep enter) 는 그대로.
  pinMode(PIN_BUZZER, OUTPUT);
  digitalWrite(PIN_BUZZER, LOW);
